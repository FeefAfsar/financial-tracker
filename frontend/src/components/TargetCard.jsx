...
export default function TargetCard({ savingTarget, targetProgress, prediction, setIsTargetModalOpen }) {
  return (
    <div className="bg-white dark:bg-zinc-900/60 backdrop-blur-md p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 w-full shadow-sm">
      <div className="flex justify-between items-center mb-4">
        ...
        <button onClick={() => setIsTargetModalOpen(true)} className="px-3 py-1.5 bg-zinc-100 dark:bg-zinc-800 text-zinc-600 dark:text-zinc-400 font-bold text-[10px] rounded-lg border border-zinc-200 dark:border-zinc-700 hover:bg-zinc-200 dark:hover:bg-zinc-750 transition-colors">CONFIG</button>
      </div>
      <div className="bg-zinc-50 dark:bg-zinc-950 p-4 rounded-xl border border-zinc-100 dark:border-zinc-850 w-full">
        <div className="flex justify-between mb-3 gap-2">
          ...
          <p className="text-xs md:text-sm font-extrabold text-emerald-600 dark:text-emerald-400 tabular-nums">Rp {savingTarget.price.toLocaleString('id-ID')}</p>
        </div>
        <div className="w-full bg-zinc-200 dark:bg-zinc-800 rounded-full h-2.5 overflow-hidden">
          <div className="bg-emerald-500 h-full rounded-full transition-all duration-500" style={{ width: `${targetProgress}%` }}></div>
        </div>
