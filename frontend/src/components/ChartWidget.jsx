...
export default function ChartWidget({ summary, theme, activePieIndex, setActivePieIndex }) {
  ...
  return (
    <div className="bg-white/80 dark:bg-zinc-900/60 backdrop-blur-md p-6 rounded-2xl border border-zinc-200 dark:border-zinc-800 w-full shadow-sm md:col-span-1 flex flex-col justify-between hover:-translate-y-1 hover:shadow-md fluid-bounce animate-dashboard-load delay-card-2">
      ...
      <div className="h-52 w-full flex items-center justify-center relative my-1">
        {summary.category_expenses.length > 0 ? (
          <>
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie 
                  ...
                  labelLine={{ strokeWidth: 1, stroke: theme === 'dark' ? '#27272a' : '#e4e4e7' }} 
                ...
                >
                  {summary.category_expenses.map((entry, index) => (
                    <Cell 
                      ...
                      className="focus:outline-none transition-all duration-300 origin-center cursor-pointer"
                      style={{
                        transform: activePieIndex === index ? 'scale(1.04)' : 'scale(1)',
                        filter: activePieIndex === index ? 'drop-shadow(0px 4px 12px rgba(16,185,129,0.15))' : 'none'
                      }}
                    />
                  ))}
                </Pie>
            ...
            {/* Teks Pusat Dinamis dengan Transisi Lembut */}
            <div className="absolute flex flex-col items-center pointer-events-none text-center px-2 max-w-[90px] transition-all duration-300">
              {activePieIndex !== -1 && summary.category_expenses[activePieIndex] ? (
                <div className="animate-dashboard-load">
