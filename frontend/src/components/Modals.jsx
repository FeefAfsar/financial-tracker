...
export function InputModal({ isOpen, onClose, formData, setFormData, handleSubmit, handleScanReceipt, isScanning, categories, showAddCategoryInput, setShowAddCategoryInput, customCategoryName, setCustomCategoryName, handleAddCustomCategory }) {
  ...
  return (
    <div className="fixed inset-0 bg-zinc-900/40 dark:bg-black/80 flex items-center justify-center p-4 z-50 w-full h-full">
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 w-full max-w-[90vw] sm:max-w-md rounded-2xl p-6 relative shadow-xl transition-colors">
         <button onClick={onClose} className="absolute top-5 right-5 text-zinc-400 hover:text-zinc-600 p-1 rounded-lg hover:bg-zinc-100"><X size={20} /></button>
         ...
         <div className="mb-5 p-3 bg-zinc-50 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-850 rounded-xl flex items-center justify-between">
           ...
           <label className={`cursor-pointer px-3 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold rounded-lg flex items-center gap-1.5 shadow-sm transition-all ${isScanning ? 'opacity-60 cursor-not-allowed scale-95' : 'active:scale-95'}`}>
         ...
         <form onSubmit={handleSubmit} className="space-y-4 text-xs font-medium">
           <div>
             ...
             <input type="number" required placeholder="Contoh: 50000" className="w-full p-3 bg-zinc-50 dark:bg-zinc-950 rounded-xl border border-zinc-200 dark:border-zinc-800 text-emerald-500 font-extrabold text-sm focus:outline-none" value={formData.amount} onChange={(e) => setFormData({...formData, amount: e.target.value})} />
           </div>
           <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <button type="button" onClick={() => setShowAddCategoryInput(!showAddCategoryInput)} className={`w-full mt-2 p-2.5 rounded-xl border font-bold text-[11px] text-center tracking-wide transition-all block active:scale-95 ${showAddCategoryInput ? 'bg-rose-50 dark:bg-rose-950/40 border-rose-200 dark:border-rose-900/20 text-rose-600 dark:text-rose-400' : 'bg-zinc-100 dark:bg-zinc-950 border-dashed border-zinc-300 dark:border-zinc-800 text-zinc-500 dark:text-zinc-400'}`}>
                  {showAddCategoryInput ? '✕ BATAL' : '+ KATEGORI BARU'}
                </button>
              </div>
           </div>
           {showAddCategoryInput && (
             <div className="p-2.5 bg-zinc-50 dark:bg-zinc-950 rounded-xl border border-zinc-200 dark:border-zinc-850 flex gap-2">
               ...
               <button type="button" onClick={handleAddCustomCategory} className="px-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-lg font-bold shrink-0 transition-colors">ADD</button>
             </div>
           )}
           <div>
             <label className="block font-bold text-zinc-400 uppercase mb-1.5">Deskripsi</label>
             <input type="text" placeholder="Rincian catatan..." className="w-full p-3 bg-zinc-50 dark:bg-zinc-950 rounded-xl border border-zinc-200 dark:border-zinc-800 text-zinc-800 dark:text-zinc-200 text-xs focus:outline-none" value={formData.description} onChange={(e) => setFormData({...formData, description: e.target.value})} />
           </div>
           <div className="flex gap-3 pt-3">
             <button type="button" onClick={onClose} className="w-1/2 p-3 bg-zinc-100 dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 text-zinc-500 rounded-xl font-bold">BATAL</button>
             <button type="submit" className="w-1/2 p-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold shadow-sm">SIMPAN</button>
           </div>
         </form>
      </div>
    </div>
...
export function DetailModal({ selectedTxDetail, onClose, isEditMode, setIsEditMode, editFormData, setEditFormData, handleEditSubmit, categories, handleDelete }) {
  ...
  return (
    <div className="fixed inset-0 bg-zinc-900/40 dark:bg-black/80 flex items-center justify-center p-4 z-50 w-full h-full" onClick={onClose}>
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 w-full max-w-sm rounded-2xl p-6 relative shadow-xl transition-all" onClick={(e) => e.stopPropagation()}>
        <button onClick={onClose} className="absolute top-5 right-5 text-zinc-400 hover:text-zinc-600 p-1 rounded-lg hover:bg-slate-100 dark:hover:bg-zinc-800"><X size={18} /></button>
        ...
        {isEditMode ? (
          <form onSubmit={handleEditSubmit} className="space-y-4 text-xs font-medium">
            ...
            <div className="pt-2 flex gap-2">
              <button type="button" onClick={() => setIsEditMode(false)} className="w-1/2 py-2.5 bg-zinc-100 dark:bg-zinc-950 text-zinc-400 rounded-xl font-bold border border-zinc-200 dark:border-zinc-800">BATAL</button>
              <button type="submit" className="w-1/2 py-2.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold shadow-sm">SIMPAN</button>
            </div>
          </form>
        ) : (
          <div className="space-y-4 text-xs font-medium">
            ...
            <div className="pt-2 flex gap-2">
              <button onClick={() => setIsEditMode(true)} className="w-1/3 py-2.5 bg-emerald-50 dark:bg-emerald-950/30 text-emerald-600 dark:text-emerald-400 rounded-xl font-bold border border-emerald-100 dark:border-emerald-900/30 hover:bg-emerald-100 transition-colors flex items-center justify-center gap-1"><Edit3 size={14} /> EDIT</button>
              <button onClick={(e) => handleDelete(selectedTxDetail.id, e)} className="w-1/3 py-2.5 bg-rose-50 dark:bg-rose-950/20 text-rose-600 dark:text-rose-400 rounded-xl font-bold border border-rose-100 hover:bg-rose-100 transition-colors flex items-center justify-center gap-1"><Trash2 size={14} /> HAPUS</button>
              <button onClick={onClose} className="w-1/3 py-2.5 bg-zinc-100 dark:bg-zinc-950 text-zinc-500 rounded-xl font-bold border border-zinc-200 dark:border-zinc-800">TUTUP</button>
            </div>
          </div>
        )}
      </div>
...
export function TargetModal({ isOpen, onClose, targetFormData, setTargetFormData, handleTargetSubmit }) {
  ...
  return (
    <div className="fixed inset-0 bg-zinc-900/40 dark:bg-black/80 flex items-center justify-center p-4 z-50 w-full h-full">
      <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 w-full max-w-[90vw] sm:max-w-md rounded-2xl p-6 relative shadow-xl transition-colors font-mono text-xs">
        <button onClick={onClose} className="absolute top-5 right-5 text-zinc-400 hover:text-zinc-600 p-1 rounded-lg hover:bg-zinc-100"><X size={20} /></button>
        ...
        <form onSubmit={handleTargetSubmit} className="space-y-4 font-sans font-medium">
           ...
           <div className="flex gap-3 pt-3">
             <button type="button" onClick={onClose} className="w-1/2 p-3 bg-zinc-100 dark:bg-zinc-950 text-zinc-500 border border-zinc-200 dark:border-zinc-800 rounded-xl font-bold">BATAL</button>
             <button type="submit" className="w-1/2 p-3 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl font-bold shadow-sm">UPDATE</button>
           </div>
        </form>
      </div>
    </div>
